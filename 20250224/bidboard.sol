...
contract BidBoard {
    ...
    // update: try passinge parameters to constructor, see note for why not calldata
    constructor(string memory _message, uint _initAmount) {
        owner = payable(msg.sender); // Set the contract deployer as the owner
        message = _message;
        currentAdvertiser = msg.sender;
        currentAmount = _initAmount;
    }
